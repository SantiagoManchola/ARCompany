...
function ThumbImg({
  ...
  const [src, setSrc] = useState<string>(srcPrimary && srcPrimary.trim() !== "" ? srcPrimary : fallbackSrc);
  return (
    <img
      src={src}
      alt={alt}
...
export default function PropertyPage() {
  ...
  // Helpers for labels and derived info
  const tipoLabel = (() => {
  ...
  const parseFloorsFromFeatures = (features?: string[]): number | null => {
    if (!features || features.length === 0) return null;
    const text = features.join(" ").toLowerCase();
    const digitMatch = text.match(/(\d+)\s+(planta|plantas|piso|pisos|nivel|niveles)/i);
    if (digitMatch) {
    ...
    for (const [w, n] of Object.entries(wordToNum)) {
      if (
        text.includes(`${w} planta`) ||
        text.includes(`${w} plantas`) ||
        text.includes(`${w} piso`) ||
        text.includes(`${w} pisos`) ||
        text.includes(`${w} nivel`) ||
        text.includes(`${w} niveles`)
      ) {
        return n;
      }
    }
  ...
  const nextImage = () => {
    setCurrentImageIndex((prev) => (prev < property.imagenes.length - 1 ? prev + 1 : prev));
  };
  ...
  const onLBMouseDown: React.MouseEventHandler<HTMLDivElement> = (e) => {
    ...
    e.preventDefault();
    setIsPanning(true);
    panStartRef.current = { x: e.clientX, y: e.clientY, startX: offset.x, startY: offset.y, moved: false };
  };
  const onLBMouseMove: React.MouseEventHandler<HTMLDivElement> = (e) => {
    ...
    const nextX = Math.max(-maxX, Math.min(maxX, panStartRef.current.startX + dx));
    const nextY = Math.max(-maxY, Math.min(maxY, panStartRef.current.startY + dy));
  ...
  const endPan = () => {
    if (panStartRef.current?.moved) {
      ...
      setTimeout(() => { justPannedRef.current = false; }, 100);
    }
    setIsPanning(false);
    panStartRef.current = null;
  };
  ...
  const onLBTouchStart: React.TouchEventHandler<HTMLDivElement> = (e) => {
    ...
    if (!t) return;
    setIsPanning(true);
    panStartRef.current = { x: t.clientX, y: t.clientY, startX: offset.x, startY: offset.y, moved: false };
  };
                ...
                {property.imagenes.length > 1 && (
                  <>
                    ...
                    <button
                      ...
                      aria-hidden={currentImageIndex >= property.imagenes.length - 1}
                      tabIndex={currentImageIndex < property.imagenes.length - 1 ? 0 : -1}
            ...
            {isLightboxOpen && typeof window !== "undefined" && createPortal(
              ...
              >
                ...
                <div className="absolute top-4 left-4 text-white space-y-1 select-none">
                  <div className="text-lg md:text-2xl font-semibold drop-shadow">{property.titulo}</div>
                  <div className="text-sm md:text-xl text-gray-100 drop-shadow">
                    {formatPrice(property.precio)}
                    {property.administracion && (
                      <>
                        {" "}+<span className="opacity-90">+ {formatPrice(property.administracion)} administración</span>
                      </>
                    )}
                  </div>
                </div>
                ...
                <div className="w-full h-full flex items-center justify-center" onClick={(e) => e.stopPropagation()}>
                  ...
                  >
                    {property.imagenes.map((img, index) => {
                      ...
                      if (zoom > 1 && index !== currentImageIndex) return null;
                      return (
                        <div
                          key={`lb-${img.id}-${index}`}
                    ...
                    {property.imagenes.length > 1 && (
                      <>
                        <button
                          onClick={(e) => { e.stopPropagation(); if (zoom <= 1) prevImage(); }}
                        ...
                        <button
                          onClick={(e) => { e.stopPropagation(); if (zoom <= 1) nextImage(); }}
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                ...
                {property.tipo && (
                  <div className="flex flex-col items-center p-4 bg-gray-50 rounded-lg">
                    <svg className="w-8 h-8 text-amber-500 mb-2" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                      {property.tipo === "APARTAMENTO" ? (
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 21h18M6 21V7a2 2 0 012-2h8a2 2 0 012 2v14M9 10h.01M9 14h.01M15 10h.01M15 14h.01" />
                      ) : (
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 10l9-7 9 7v9a2 2 0 01-2 2h-4a2 2 0 01-2-2v-5H9v5a2 2 0 01-2 2H5a2 2 0 01-2-2v-9z" />
                      )}
                    </svg>
                ...
                {/* Habitaciones */}
                <div className="flex flex-col items-center p-4 bg-gray-50 rounded-lg">
                  <svg className="w-8 h-8 text-amber-500 mb-2" fill="currentColor" viewBox="0 0 640 512">
                    <path d="M32 32c17.7 0 32 14.3 32 32V320H288V160c0-17.7 14.3-32 32-32H544c53 0 96 43 96 96V448c0 17.7-14.3 32-32 32s-32-14.3-32-32V416H352 320 64v32c0 17.7-14.3 32-32 32s-32-14.3-32-32V64C0 46.3 14.3 32 32 32zm144 96a80 80 0 1 1 0 160 80 80 0 1 1 0-160z"/>
                  </svg>
                  <span className="text-2xl font-bold text-gray-900">{property.alcobas}</span>
                  <span className="text-sm text-gray-600">Habitaciones</span>
                </div>

                {/* Baños */}
                <div className="flex flex-col items-center p-4 bg-gray-50 rounded-lg">
                  <svg className="w-8 h-8 text-amber-500 mb-2" fill="currentColor" viewBox="0 0 512 512">
                  ...
                  <span className="text-2xl font-bold text-gray-900">{property.banos}</span>
                ...
                <div className="flex flex-col items-center p-4 bg-gray-50 rounded-lg">
                  ...
                  <span className="text-2xl font-bold text-gray-900">{property.garajes}</span>
                ...
                {property.tipo === "APARTAMENTO" && property.piso && (
                  <div className="flex flex-col items-center p-4 bg-gray-50 rounded-lg">
                    <svg className="w-8 h-8 text-amber-500 mb-2" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 20h16M6 16h4v-3h4v-3h4V6" />
                    </svg>
                    <span className="text-2xl font-bold text-gray-900">{property.piso}</span>
                ...
                {property.tipo === "CASA" && (
                  <div className="flex flex-col items-center p-4 bg-gray-50 rounded-lg">
                    ...
                    <span className="text-2xl font-bold text-gray-900">{property.pisos ?? (() => {
                      const text = (property.caracteristicas || []).join(" ").toLowerCase();
                      const m = text.match(/(\d+)\s+(planta|plantas|piso|pisos|nivel|niveles)/i);
                ...
                {property.estrato && (
                  <div className="flex flex-col items-center p-4 bg-gray-50 rounded-lg">
                    ...
                    <span className="text-2xl font-bold text-gray-900">{property.estrato}</span>
                ...
                {property.antiguedad && (
                  <div className="flex flex-col items-center p-4 bg-gray-50 rounded-lg">
                    ...
                    <span className="text-2xl font-bold text-gray-900">{property.antiguedad}</span>
